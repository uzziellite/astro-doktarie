<template>
	<div class="w-full mb-4" v-if="!showModal">
    <label for="search" class="block font-semibold text-stone-700 dark:text-white uppercase">
      Find a product
    </label>
    <div class="mt-1 flex rounded-md shadow-sm" @click="showModal = ! showModal">
      <input type="text" class="flex-1 block w-full rounded-none rounded-l-md sm:text-sm border-gray-300 dark:bg-stone-800 cursor-pointer" placeholder="Search product..." disabled>
	    ...
	    <div class="flex items-end sm:items-center justify-center min-h-full p-4 text-center sm:p-0">
	      <div class="relative bg-white dark:bg-black rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-xl sm:w-full">
	        <div class="bg-white dark:bg-black pt-5 pb-4">
	          <form @submit.prevent="searchProduct">
					    <div class="grid grid-cols-3 gap-6 p-2">
					      <div class="col-span-3">
					        <label class="block font-semibold text-stone-700 dark:text-white uppercase">
					          Find a product
					        </label>
					        <div class="mt-1 flex rounded-md shadow-sm">
					          ...
					          <span class="inline-flex items-center px-6 rounded-r-md border border-l-0 border-gray-300 bg-doktarie-blue text-white text-sm cursor-pointer" @click="searchProduct">
              ...
              <a :href="`/equipment/for-${type}/details/${item.slug}`" v-for="item in data" :key="item.id" class="text-stone-800 dark:text-white font-semibold px-3 py-2 flex items-center capitalize bg-stone-200 dark:bg-stone-600">
                {{item.name}}
              </a>
            </div>
...
export default{
	...
	methods:{
		searchProduct() {
			...
			fetch(`${this.url}items/equipment?filter=%7B%22status%22:%7B%22_eq%22:%22published%22%7D%7D&fields[]=name,slug,images.directus_files_id,category.name&limit=4&sort=-date_created&search=${this.search}`, {
        method: 'GET',
        headers: {
